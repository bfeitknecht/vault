...
var require_path = __commonJS({
  "node_modules/path/path.js"(exports2, module2) {
    ...
    function win32SplitPath(filename) {
      ...
      var result2 = splitTailRe.exec(tail), dir = result2[1], basename5 = result2[2], ext = result2[3];
      return [device, dir, basename5, ext];
    }
    ...
    posix8.normalize = function(path) {
      var isAbsolute = posix8.isAbsolute(path), trailingSlash = path && path[path.length - 1] === "/";
    ...
    posix8.isAbsolute = function(path) {
      return path.charAt(0) === "/";
    };
    posix8.join = function() {
      var path = "";
      for (var i = 0; i < arguments.length; i++) {
      ...
      return posix8.normalize(path);
    };
    posix8.relative = function(from, to) {
      from = posix8.resolve(from).substr(1);
      to = posix8.resolve(to).substr(1);
    ...
    posix8._makeLong = function(path) {
      return path;
    };
    posix8.dirname = function(path) {
    ...
    posix8.basename = function(path, ext) {
    ...
    posix8.extname = function(path) {
      return posixSplitPath(path)[3];
    };
    posix8.format = function(pathObject) {
      if (!util.isObject(pathObject)) {
        throw new TypeError(
          "Parameter 'pathObject' must be an object, not " + typeof pathObject
        );
      ...
      var dir = pathObject.dir ? pathObject.dir + posix8.sep : "";
      var base = pathObject.base || "";
      return dir + base;
    };
    posix8.parse = function(pathString) {
      if (!util.isString(pathString)) {
        throw new TypeError(
          "Parameter 'pathString' must be a string, not " + typeof pathString
        );
    ...
    if (isWindows)
      ...
      module2.exports = posix8;
    module2.exports.posix = posix8;
...
async function getAttachmentFolderFullPath(plugin, substitutions) {
...
// src/Vault.ts
var { join: join2 } = import_path2.posix;
...
var { extname, basename, dirname } = import_path3.posix;
...
async function collectAttachments(plugin, note, oldPath, attachmentFilter) {
  ...
  const notice = new import_obsidian6.Notice(`Collecting ${attachmentsMap.size} attachments for ${note.path}`);
...
async function prepareAttachmentToMove(plugin, link, newNotePath, oldNotePath) {
  ...
  const oldNoteBaseName = basename3(oldNotePath, extname3(oldNotePath));
  const newNoteBaseName = basename3(newNotePath, extname3(newNotePath));
  ...
  if (shouldRemoveNewAttachmentFolder) {
    await app.vault.adapter.rmdir(dirname2(newAttachmentPath), false);
  }
...
async function collectAttachmentsInFolder(plugin, folder) {
  ...
  const files = [];
  import_obsidian6.Vault.recurseChildren(folder, (child) => {
    if (isNote(child)) {
      files.push(child);
    }
  });
...
async function updateLinksInFile(app, file, oldPath) {
...
async function updateLink(app, link, file, source) {
  if (!file) {
  ...
  if (isOldFileRenamed) {
    shouldRemoveNewAttachmentFolder = await createFolderSafe(app, dirname3(newPath));
  ...
  if (newPath && shouldRemoveNewAttachmentFolder) {
    await app.vault.adapter.rmdir(dirname3(newPath), false);
  }
  return newLink;
}
function convertLink(app, link, source, oldPath) {
  oldPath ??= source.path;
...
async function fillRenameMap(plugin, file, oldPath) {
  ...
  const oldNoteName = basename4(oldPath, extname4(oldPath));
...
async function processRename(plugin, oldPath, newPath) {
  ...
  if (oldFile) {
    await createFolderSafe(app, dirname3(newPath), plugin.settings.keepEmptyAttachmentFolders);
...
function getAlias(app, displayText, oldFile, newPath, sourcePath) {
  ...
  for (const path of [oldFile.path, newPath]) {
    ...
    const fileNameWithExtension = basename4(path);
    const fileNameWithoutExtension = basename4(path, extension);
...
// src/CustomAttachmentLocationPlugin.ts
var { join: join5 } = import_path7.posix;
var CustomAttachmentLocationPlugin = class extends import_obsidian8.Plugin {
  _settings;
  get settings() {
