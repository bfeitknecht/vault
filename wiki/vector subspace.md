...
Two vector spaces $U, V$ are said to be **orthogonal**, $U \mathop\bot V$ if it holds that $\forall \mathbf{u} \in U, \mathbf{v}  \in V : \mathbf{u}^{\mathsf{T}} \mathbf{v} = 0$, i.e. all vectors are orthogonal.
