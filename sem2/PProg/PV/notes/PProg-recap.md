...
`ExecutorService` methods: {1: `submit(Thread)`} to start a task's implemented {2: `void run()` or `T call()`} method, which can then {3: `submit()`} to spawn another task and {4: `t.get()`} (need to wrap in `try{}catch(){}` block!) to try to obtain the {5: `Future<T>`} that is the result of the spawned task.
