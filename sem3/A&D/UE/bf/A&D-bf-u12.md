...
Let $e_{1} \in T_{1} - T_{2}$ be an edge in the respective set difference of the two MST.  Then, $F_{1} = T_{1} - \{ e_{1} \}$ is the forest resulting from removing the edge $e_{1}$, where $F_{1}  = X_{1} \cup Y_{1}$ are its two connected components.

As $T_{2}$ is an MST, there must exist an edge $e_{2} = \{ x_{2}, y_{2} \} \in T_{2}$ between the two connected components $X_{1}, Y_{1}$. Together, these cover all edges in $T_{1}$, therefore the edge cannot belong to $T_{1}$, i.e. $e_{2} \in T_{2} - T_{1}$. Thus $T_{1}' = F_{1} \cup \{ e_{2} \}$ is connected and hence a spanning tree.

Now consider $F_{2} = T_{2} - \{ e_{2} \}$. As $T_{1}$ is MST, 
