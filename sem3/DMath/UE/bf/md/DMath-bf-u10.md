...
However, we have $x^{26} \equiv_{m(x)} 1$. Thus, since the order of the subgroup generated by $x$ is equal to that of the multiplicative group of the underlying field, $|H| = 26 = |F^{*}|$ and the subgroup is a subset of the mutliplicative group of units $H \subseteq F^{*}$, it must be that $H = F^{*}$. Hence $x$ is a generator of $F^{*}$.
$\square$
