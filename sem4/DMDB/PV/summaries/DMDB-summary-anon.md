...
$R \Join S$: Combine all elements in which all common attributes have same values, no duplicates
$R \rtimes S$: Same as $\Join$ , but only keeps right attributes
$R \ltimes S$: Same as $\Join$ , but only keeps left attributes
$R \Join_{\theta} S$: Does **cartesian product** $\times$ and then filters for condition $\theta$
$R \Join_{A} S$: Same as $\Join$ but only matches column $A$ (subclass of theta-join $\Join_{\theta}$)
...
$R\mathop{⟖}S$: Same as $\Join$, but includes all rows from $S$ and fills in missing values from $R$ with nulls.
$R\mathop{⟕}S$: Same as $\Join$, but includes all rows from $R$ and fills in missing values from $S$ with nulls.
